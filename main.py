...
DEFAULT_YAML_CONFIG = """# Checker By: @DEVELOPER_RAIM
# Telegram: https://t.me/DEVELOPER_RAIM
...
by @DEVELOPER_RAIM | Telegram: https://t.me/DEVELOPER_RAIM
...
DISCORD_WEBHOOK_USERNAME = "Netflix Checker by @DEVELOPER_RAIM"
...
def _resolve_update_endpoints():
    # ref: original author, kept for functionality
...
def format_cookie_file(info, cookie_content, config, is_subscribed, nftoken_data=None):
    ...
    lines.append(divider)
    lines.append("")
    lines.append("Checker By: @DEVELOPER_RAIM | Telegram: https://t.me/DEVELOPER_RAIM")
...
def build_discord_full_message(config, info, is_subscribed, output_filename, nftoken_data=None, use_emojis=True):
    lines = ["# [Netflix Cookie](https://t.me/DEVELOPER_RAIM)", "", "Cookie details"]
    ...
    lines.extend(
        [
            "",
            "**[Telegram](https://t.me/DEVELOPER_RAIM)** | Contact: @DEVELOPER_RAIM",
        ]
    )
    return "\n".join(lines)
...
def build_discord_cookie_message(cookie_content):
    lines = [
        "# [Netflix Cookie](https://t.me/DEVELOPER_RAIM)",
        "",
        "Cookie details",
        "```txt",
...
def build_discord_nftoken_message(info, nftoken_data, nftoken_mode, use_emojis=True):
    ...
    lines = ["# [Netflix NFToken](https://t.me/DEVELOPER_RAIM)", ""]
...
def build_telegram_full_message(config, info, is_subscribed, output_filename, nftoken_data=None, use_emojis=True):
    lines = ['<b><a href="https://t.me/DEVELOPER_RAIM">Netflix Cookie</a></b>', "", "<b>Cookie details</b>"]
    for line in build_notification_details(config, info, is_subscribed, output_filename):
        label, value = line.split(": ", 1)
    ...
    lines.extend(
        [
            "",
            '<b><a href="https://t.me/DEVELOPER_RAIM">Telegram</a></b> | Contact: @DEVELOPER_RAIM',
        ]
    )
    return "\n".join(lines)
...
def build_telegram_cookie_message(cookie_content):
    lines = [
        '<b><a href="https://t.me/DEVELOPER_RAIM">Netflix Cookie</a></b>',
        "",
        "<b>Cookie details</b>",
...
def build_telegram_nftoken_message(info, nftoken_data, nftoken_mode, use_emojis=True):
    ...
    lines = ['<b><a href="https://t.me/DEVELOPER_RAIM">Netflix NFToken</a></b>', ""]
    links = build_nftoken_links((nftoken_data or {}).get("token"), nftoken_mode) if has_usable_nftoken(nftoken_data) else []
    if links:
...
def check_cookies(num_threads=30, config=None):
    ...
    def handle_result(info, netscape_content, cookie_path, cookie_file, is_subscribed, cookie_dict, remove_source=True):
        ...
        if is_subscribed:
            ...
            filename = f"{max_streams}_{country}_by_DEVELOPER_RAIM_{info.get('showExtraMemberSection')}_{user_guid}_{random_suffix}.txt"
        else:
            has_payment_method = "True" if decode_netflix_value(info.get("paymentMethodType")) not in {None, "", "UNKNOWN", "Unknown", "N/A"} else "False"
            filename = f"PaymentM-{has_payment_method}_{country}_by_DEVELOPER_RAIM_{user_guid}_{random_suffix}.txt"
